...
#     transfer_comment='' : varchar(1536)  # short description
#     score:   float  # loss
...
#
    ...
    #     if key is None:
    #         key = self.fetch1('KEY')
    #     model_fn, model_config = (Model.proj(transfer_model_fn='model_fn',
    #                                          transfer_model_config='model_config',
    #                                          transfer_model_hash='model_hash')
    #                               & key).fetch1('transfer_model_fn', 'transfer_model_config')
    #     dataset_fn, dataset_config = (Dataset.proj(transfer_dataset_fn='dataset_fn',
    #                                                transfer_dataset_hash='dataset_hash',
    #                                                transfer_dataset_config='dataset_config')
    #                                   & key).fetch1('transfer_dataset_fn', 'transfer_dataset_config')
    #     trainer_fn, trainer_config = (Trainer.proj(transfer_trainer_fn='trainer_fn',
    #                                                transfer_trainer_hash='trainer_hash',
    #                                                transfer_trainer_config='trainer_config')
    #                                   & key).fetch1('transfer_trainer_fn', 'transfer_trainer_config')
    #
...
#                 comments = []
#                 comments.append((Trainer.proj(transfer_trainer_fn='trainer_fn', transfer_trainer_hash='trainer_hash',
#                                               transfer_trainer_comment='trainer_comment')
#                                  & key).fetch1("transfer_trainer_comment"))
#                 comments.append((Model.proj(transfer_model_fn='model_fn', transfer_model_hash='model_hash',
#                                             transfer_model_comment='model_comment')
#                                  & key).fetch1("transfer_model_comment"))
#                 comments.append((Dataset.proj(transfer_dataset_fn='dataset_fn', transfer_dataset_hash='dataset_hash',
#                                               transfer_dataset_comment='dataset_comment')
#                                  & key).fetch1("transfer_dataset_comment"))
#                 key['transfer_comment'] = '.'.join(comments)
#                 self.insert1(key)
#
