...
class RepresentationAnalyser:
    ...
    def _cosine_loss(self, rep_1, rep_2):
        ...
        return cosine_criterion(rep_1, rep_2, torch.ones(rep_1.shape[:1], device=self.device))
    ...
    def _plot_dim_reduction(self, df, data_columns, num_labels=100, hue="y", style=None, title="", file_name="",
                            legend=False, acc=None):
        ...
        for i, (x, y) in enumerate(data_columns):
            ...
            if acc:
                ax[i].text(0.85, 0.90, "Accuracy: {:02.2f}".format(acc), transform=ax[i].transAxes)
        sns.despine(offset=10, trim=True)
        if title:
            fig.suptitle(title, fontsize=16)
        if file_name:
            fig.savefig(os.path.join(self.path, file_name), facecolor=fig.get_facecolor(),
    ...
    def dim_reduction(self, method="tsne", mode="combined", noise_level=0.0, pca=None):
        ...
        self._plot_dim_reduction(df, data_columns,
    ...
    def _plot_corr_matrix(self, mat, title="", file_name="", n_clusters=10, indices=None, acc=None):
        ...
        if title:
            fig.suptitle(title, fontsize=16)
        if acc:
            ax.text(0.82, 0.93, "Accuracy: {:02.2f}".format(acc), transform=ax.transAxes)
        if file_name:
            fig.savefig(os.path.join(self.path, file_name), facecolor=fig.get_facecolor(),
                        edgecolor=fig.get_edgecolor(), bbox_inches='tight')
            plt.close(fig)
