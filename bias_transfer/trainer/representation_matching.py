...
class RepresentationMatching(NoiseAugmentation):
    ...
    def pre_forward(self, model, inputs, shared_memory, train_mode):
        ...
        if self.config.representation_matching.get("second_noise_std", None) \
            ...
            inputs2, _ = self.apply_noise(inputs[self.clean_flags],
                                          self.device,
    ...
    def post_forward(self, outputs, loss, extra_losses, applied_std, **kwargs):
        rep_1 = outputs[self.rep][:self.batch_size][self.clean_flags]
        rep_2 = outputs[self.rep][self.batch_size:]
        if self.config.representation_matching.get("criterion", "cosine") == "cosine":
            o = torch.ones(rep_1.shape[:1], device=self.device)  # ones indicating that we want to measure similarity
