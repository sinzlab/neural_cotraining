...
class NoiseAugmentation(MainLoopModule):
    ...
    def apply_noise(self, x, device, std: dict = None, snr: dict = None, rnd_gen=None):
        with torch.no_grad():
            if std:
                ...
                noise_levels = {-1: 1.0}
            assert abs(sum(noise_levels.values()) - 1.0) < 0.00001, "Percentage for noise levels should sum to one!"
            ...
            for level, percentage in noise_levels.items():  # TODO: is this efficient enough?
                ...
                if isinstance(level, tuple):  # select level randomly from range
                    level = torch.empty(1, device=device, dtype=torch.float32).uniform_(level[0], level[1])
    ...
    def pre_epoch(self, model, train_mode):
        if not train_mode:
            ...
            self.rnd_gen = rnd_gen.manual_seed(self.seed)  # so that we always have the same noise for evaluation!

    def pre_forward(self, model, inputs, shared_memory, train_mode):
        inputs, shared_memory["applied_std"] = self.apply_noise(inputs,
                                                                self.device,
